...
def check_dependencies(method="pytorch"):
    ...
    # MLX特有の依存関係
    mlx_dependencies = {"mlx": "pip install mlx mlx-lm"}
...
def fine_tune_with_mlx(
    data_path: str, output_path: str, iterations: int = 100, model_name: str = None
):
    """MLXを使用したfine-tuning"""
...
"""
    ...
    print(f"\n実行コマンド:\n{cmd}")
    print_status(
        "注: MLXのインストールが必要です。エラーが出た場合は以下を実行してください:", "warning"
    )
    ...
    if result.returncode == 0:
        ...
        print_status(
            "代替案: PyTorchを使用したfine-tuningを試してください (--method pytorch)", "info"
        )
        return False
...
def main():
    ...
    parser.add_argument(
        "--method", choices=["mlx", "pytorch"], default="mlx", help="Fine-tuning手法の選択"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument("--iterations", type=int, default=100, help="トレーニングイテレーション数")
    parser.add_argument(
        "--output-dir", type=str, default="./finetuned_model", help="出力ディレクトリ"
    )
